...
const TX_COLOR = { deposit: '#22c55e', withdraw: '#f59e0b', swap: '#06b6d4', rebalance: '#8b5cf6', yield_payment: '#f59e0b' }
...
const TX_TRIGGER = {
  ...
  rebalance:     'drift',
  swap:          'hedge',
  yield_payment: 'threshold_crossed',
}
        ...
        <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 11 }}>
          <thead>
            <tr style={{ borderBottom: '1px solid #1e293b' }}>
              {['TYPE','PROTOCOL','AMOUNT','TRIGGER','TX HASH','STATUS','TIME'].map(h => (
          ...
          <tbody>
            {loading ? Array(5).fill(0).map((_, i) => (
              ...
              <tr><td colSpan={7} style={{ padding: 20, textAlign: 'center', color: '#334155', fontSize: 12 }}>No transactions yet</td></tr>
            ) : rows.map((tx, i) => {
              ...
              return (
                <tr key={i} style={{ borderBottom: '1px solid #0f172a' }}>
                  ...
                  <td style={{ padding: '7px 10px', color: '#64748b', fontSize: 9, letterSpacing: '0.05em' }}>
                    {tx.trigger ?? TX_TRIGGER[tx.tx_type] ?? '—'}
                  </td>
